...
n_pnts = length(cs_n);
xbulk_n = zeros(param.Nc,n_pnts);
xbulk_p = zeros(param.Nc,n_pnts);
soc_bulk_n = zeros(param.Nc,n_pnts);
soc_bulk_p = zeros(param.Nc,n_pnts);

for j = 1:length(cs_n) %NOTE: Changed this from param.t_data for ODE event function handling
for i = 1:param.Nc

%shift_R = (i-1)*(param.Nr-1);
xbulk_n(i,j) = trapz(soc_int_n,soc_int_n.^2.*cs_n)*3/(param.c_n_max*param.Rs_n^3); % Pulled out shiftR term assuming it related to multiple cells
xbulk_p(i,j) = trapz(soc_int_p,soc_int_p.^2.*cs_p)*3/(param.c_p_max*param.Rs_p^3);
