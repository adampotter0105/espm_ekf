...
[V_real, ~, ~, ~, SOC_n_real, SOC_p_real, cs_n_real, cs_p_real,...
          ~, ~,~, ~,~, ~, ~, ~, ~, ~,...
          ~, ~, ~, ~, ~, ~, ~] = ESPM_sim(0, 0, param.t_data, param.I_data, ...
...
[cs_initial_noisy,csn0,csp0] = conc_initial_sd((1+x0_error_factor)*soc_variation(param), param);
x_initial_noisy = [cs_initial_noisy; ce_initial; T_initial; sei_initial; Q_initial; aina_initial_n; aina_initial_p; c_sei_initial; c_li_initial; film_initial; Csolv_initial];
...
for i=2:length(tspan)
    ...
    tic
    predict(ekf,DT);
    estimateStates_ekf(:,i) = correct(ekf,V_real(i));
    runtimes1(i) = toc;
    fprintf("EKF step: %d \n",i-1)
end
...
%% Run UKF
estimateStates_ukf = NaN(n_states,numSteps); % preallocate state estimates
...
for i=2:length(tspan)
    tic
    predict(ukf,DT);
    estimateStates_ukf(:,i) = correct(ukf,V_real(i));
    runtimes2(i) = toc;
    fprintf("UKF step: %d \n",i-1)
end
